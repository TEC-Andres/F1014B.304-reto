...
_APA_CSL_URL        = None
# Use the supplied local APA CSL file at apa.csl (cached from CSL repo on 2024-06-01)
_APA_CSL_CACHE_PATH = Path(__file__).resolve().parent / "apa.csl"
...
class LatexProjectCompiler:
    ...
    def _biber(self) -> subprocess.CompletedProcess:
        # Use cwd for paths to avoid biber issues with non-ASCII absolute paths on Windows.
        return self._run([
            "biber",
            self.main_tex.stem,
        ])
